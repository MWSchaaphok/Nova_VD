function [f,sp1,sp2,sp3,handles] = plotUIfigure(S_nr,Sector,var_list, lap)
...
%% Lap dropdown menu's
    string = {};
    for j = 1:length(lap.ind)
        string{end+1} = strcat('Lap ',num2str(j));
    end
    % Subplot 1 
    handles.lapf1 = uicontrol(panel,'style','popupmenu');
    handles.lapf1.Position = [305 280 100 20];
    handles.lapf1.String = string;
    
        % Subplot 2
    handles.lapf2 = uicontrol(panel,'style','popupmenu');
    handles.lapf2.Position = [305 220 100 20];
    handles.lapf2.String = string;
    
        % Subplot 3 
    handles.lapf3 = uicontrol(panel,'style','popupmenu');
    handles.lapf3.Position = [305 160 100 20];
    handles.lapf3.String = string;
...
%% Callback functions
    function refresh(src,event)
       ...
       lapf1  = handles.lapf1.Value;
       lapf2  = handles.lapf2.Value;
       lapf3  = handles.lapf3.Value; 
       ...
       if S_nr == 0 || nr_sect_plot == S_nr
           if strcmp(plot1x,'Track')
               plotTrack(plot1y,sp1,1,handles,lap,lapf1)
           elseif strcmp(plot1x,'Distance')
               plotDistance_sector(plot1y,sp1,1,handles,lap,lapf1)
           elseif strcmp(plot1x, 'Time')
               plotTime_sector(plot1y,sp1,1,handles,lap,lapf1)
           else
               disp(' This choice is not possible, please select another option.\n');
           end 

           if strcmp(plot2x,'Track')
               plotTrack(plot2y,sp2,2,handles,lap,lapf2)
           elseif strcmp(plot2x,'Distance')
               plotDistance_sector(plot2y,sp2,2,handles,lap,lapf2)
           elseif strcmp(plot2x, 'Time')
               plotTime_sector(plot2y,sp2,2,handles,lap,lapf2)
           else
               disp(' This choice is not possible, please select another option.\n');
           end 

           if strcmp(plot3x,'Track')
               plotTrack(plot3y,sp3,3,handles,lap,lapf3)
           elseif strcmp(plot3x,'Distance')
               plotDistance_sector(plot3y,sp3,3,handles,lap,lapf3)
           elseif strcmp(plot3x, 'Time')
               plotTime_sector(plot3y,sp3,3,handles,lap,lapf3)
           else
               disp(' This choice is not possible, please select another option.\n');
           end 
           hold off
           ...
           fprintf('We plot only the selected sectors...\n')
            if strcmp(plot1x,'Track')
               plotTrack(plot1y,sp1,1,handles,lap,lapf1)
           elseif strcmp(plot1x,'Distance')
               plotDistance_sector(plot1y,sp1,1,handles,lap,lapf1,Sector,Sect_plot)
           elseif strcmp(plot1x, 'Time')
               plotTime_sector(plot1y,sp1,1,handles,lap,lapf1,Sector,Sect_plot)
           else
               disp(' This choice is not possible, please select another option.\n');
           end 
           ...
           if strcmp(plot2x,'Track')
               plotTrack(plot2y,sp2,2,handles,lap,lapf2)
           elseif strcmp(plot2x,'Distance')
               plotDistance_sector(plot2y,sp2,2,handles,lap,lapf2,Sector,Sect_plot)
           elseif strcmp(plot2x, 'Time')
               plotTime_sector(plot2y,sp2,2,handles,lap,lapf2,Sector,Sect_plot)
           else
               disp(' This choice is not possible, please select another option.\n');
           end 
           ...
           if strcmp(plot3x,'Track')
               plotTrack(plot3y,sp3,3,handles,lap,lapf3)
           elseif strcmp(plot3x,'Distance')
               plotDistance_sector(plot3y,sp3,3,handles,lap,lapf3,Sector,Sect_plot)
           elseif strcmp(plot3x, 'Time')
               plotTime_sector(plot3y,sp3,3,handles,lap,lapf3,Sector,Sect_plot)
           else
               disp(' This choice is not possible, please select another option.');
           end 
           hold off          
    ...
    function extract(src,event,sbpl)
        ...
        if strcmp(graph,'All')
            if strcmp(plotx,'Track')
                plotTrack(ploty,SP,4,handles,lap,lapf)
            elseif strcmp(plotx,'Distance')
                plotDistance_sector(ploty,SP,4,handles,lap,lapf)
            elseif strcmp(plotx, 'Time')
                plotTime_sector(ploty,SP,4,handles,lap,lapf)
            else
                disp(' This choice is not possible, please select another option.');
            end 
        else 
            if strcmp(plotx,'Track')
               updateTrack(ploty,graph,SP,lap,lapf)
           elseif strcmp(plotx,'Distance')
               updateDistance_sector(ploty,graph,SP,lap,lapf)
           elseif strcmp(plotx, 'Time')
               updateTime_sector(ploty,graph,SP,lap,lapf)
           end
        end
    end
       ...
       for i = 1:S_nr
        ...
        if S_nr == 0 || nr_sect_plot == S_nr 
             if strcmp(graph,'All')
               ...
               if strcmp(plotx,'Track')
                   plotTrack(ploty,sp,sbpl,handles,lap,lapf)
               elseif strcmp(plotx,'Distance')
                   plotDistance_sector(ploty,sp,sbpl,handles,lap,lapf)
               elseif strcmp(plotx, 'Time')
                   plotTime_sector(ploty,sp,sbpl,handles,lap,lapf)
               else
                   disp(' This choice is not possible, please select another option.');
               end 
            ...
            if strcmp(graph,'All')
               ...
               if strcmp(plotx,'Track')
                   plotTrack(ploty,sp,sbpl,handles,lap,lapf)
               elseif strcmp(plotx,'Distance')
                   plotDistance_sector(ploty,sp,sbpl,handles,lap,lapf,Sector,Sect_plot)
               elseif strcmp(plotx, 'Time')
                   plotTime_sector(ploty,sp,sbpl,handles,lap,lapf,Sector,Sect_plot)
               else
                   disp(' This choice is not possible, please select another option.');
               end 
               ...
               if strcmp(plotx,'Track')
                   updateTrack(ploty,graph,sp,lap,lapf)
               elseif strcmp(plotx,'Distance')
                   updateDistance_sector(ploty,graph,sp,lap,lapf,Sector,Sect_plot)
               elseif strcmp(plotx, 'Time')
                   updateTime_sector(ploty,graph,sp,lap,lapf,Sector,Sect_plot)
               end
             end
        end
    end
