function [] = updateTime_sector(ploty,graph,sp,lap,lap_nr,Sector,Sect_plot)
    ...
    elseif nargin == 7
        ...
        for j= Sect_plot
            ...
            plot(sp, var.t(Sector.(name).ind(lap_start:lap_end)), var.(graph)(Sector.(name).ind(lap_start:lap_end)),'b')
            hold(sp, 'on'); 
        end 
        ttl = strrep(ploty,'_',' ');
