function [tc,lapt,lapstr] = Laptime2(d_dis,v_dec,curv3,curv4,n_lap,par)
...
% % Compute lap 1
% t_lap = d_dis(2:size(curv3,2))./v_dec(2:size(curv3,2));
%        tc2 = cumsum(t_lap);
%        lapt(1) = tc2(end);
...
% %Compute other laps
% for idx = 2:n_lap
%        t_lap = d_dis( 2 + ((size(curv3,2)-size(curv3,2))/...
%           (n_lap-1))*(idx-1):...
